...
impl<'a, 'tcx> EmbargoVisitor<'a, 'tcx> {
    fn ty_level(&self, ty: &hir::Ty) -> Option<AccessLevel> {
        let def = match ty.node {
            hir::TyPath(_, ref path) => path.def,
            hir::TyProject(..) => self.tcx.tables().project_defs[&ty.id],
...
impl<'b, 'a, 'tcx: 'a> ReachEverythingInTheInterfaceVisitor<'b, 'a, 'tcx> {
    ...
    fn reach_aliased_type(&mut self, item: &hir::Item, segment: &hir::PathSegment) {
...
impl<'b, 'a, 'tcx: 'a, 'v> Visitor<'v> for ReachEverythingInTheInterfaceVisitor<'b, 'a, 'tcx> {
    fn visit_ty(&mut self, ty: &hir::Ty) {
        let def_and_segment = match ty.node {
            hir::TyPath(_, ref path) => {
                Some((path.def, path.segments.last().unwrap()))
            }
            hir::TyProject(_, ref segment) => {
                Some((self.ev.tcx.tables().project_defs[&ty.id], &**segment))
            }
            _ => None
        };
...
impl<'a, 'tcx, 'v> Visitor<'v> for PrivacyVisitor<'a, 'tcx> {
    ...
    fn visit_expr(&mut self, expr: &hir::Expr) {
        match expr.node {
            ...
            hir::ExprStruct(ref path, ref expr_fields, _) => {
                let adt = self.tcx.tables().expr_ty(expr).ty_adt_def().unwrap();
                let variant = adt.variant_of_def(path.def);
...
impl<'a, 'tcx: 'a, 'v> Visitor<'v> for SearchInterfaceForPrivateItemsVisitor<'a, 'tcx> {
    fn visit_ty(&mut self, ty: &hir::Ty) {
        let def_and_segment = match ty.node {
            ...
            hir::TyProject(_, ref segment) => {
                Some((self.tcx.tables().project_defs[&ty.id], &**segment))
            }
            _ => None
        };
        if let Some((def, segment)) = def_and_segment {
