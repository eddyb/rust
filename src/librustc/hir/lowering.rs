...
pub trait Resolver {
    ...
    // Remove the resolution associated with a node id
...
impl<'a> LoweringContext<'a> {
    ...
    fn lower_view_path(&mut self, id: NodeId, view_path: &ViewPath) -> P<hir::ViewPath> {
        P(Spanned {
            node: match view_path.node {
                ViewPathSimple(ident, ref path) => {
                    hir::ViewPathSimple(ident.name,
                                        self.lower_path(id, path, ParamMode::Explicit))
                }
                ViewPathGlob(ref path) => {
                    hir::ViewPathGlob(self.lower_path(id, path, ParamMode::Explicit))
                }
                ViewPathList(ref path, ref path_list_idents) => {
                    hir::ViewPathList(self.lower_path(id, path, ParamMode::Explicit),
    ...
    fn lower_path_list_item(&mut self, path_list_ident: &PathListItem) -> hir::PathListItem {
        hir::PathListItem {
            ...
            def: self.expect_full_def(path_list_ident.node.id),
        ...
        P(hir::Ty {
            ...
            node: match t.node {
                ...
                TyKind::Path(ref qself, ref path) => {
                    self.lower_qpath(t.id, qself, path, ParamMode::Explicit,
    ...
    fn lower_qpath<F, G, T>(&mut self,
                            ...
                            mk_project: G)
                            -> T
    where F: Fn(Option<P<hir::Ty>>, hir::Path) -> T,
          G: Fn(P<hir::Ty>, P<hir::PathSegment>) -> T
    {
        ...
        if resolution.depth == 0 {
            return mk_path(qself, path);
        }

        // Don't keep the inner resolution for the outer node.
        self.resolver.clear_resolution(id);

        // Create the innermost type that we're projecting from.
        let mut ty = if path.segments.is_empty() {
            qself.expect("missing QSelf for <T>::...")
        } else {
            let ty = self.ty(p.span, hir::TyPath(qself, path));
        ...
        for (i, segment) in p.segments.iter().enumerate().skip(proj_start) {
            ...
            // It's finished, return the projection of the right node type.
            if i == p.segments.len() - 1 {
                return mk_project(ty, segment);
            }

            // Wrap in another projection type.
            ty = self.ty(p.span, hir::TyProject(ty, segment));
        }

        // Should've returned in the for loop above.
        span_bug!(p.span, "lower_qpath: no final projection segment in {}..{}",
    ...
    fn lower_item_kind(&mut self, id: NodeId, i: &ItemKind) -> hir::Item_ {
        match *i {
            ItemKind::ExternCrate(string) => hir::ItemExternCrate(string),
            ItemKind::Use(ref view_path) => {
                hir::ItemUse(self.lower_view_path(id, view_path))
            }
    ...
    pub fn lower_item(&mut self, i: &Item) -> hir::Item {
        let node = self.with_parent_def(i.id, |this| {
            this.lower_item_kind(i.id, &i.node)
        });
        ...
        P(hir::Pat {
            ...
            node: match p.node {
                ...
                PatKind::Ident(ref binding_mode, pth1, ref sub) => {
                    self.with_parent_def(p.id, |this| {
                        match this.resolver.get_resolution(p.id).map(|d| d.base_def) {
                            ...
                            Some(def) => {
                                hir::PatKind::Path(None, hir::Path {
                                    span: pth1.span,
                                    global: false,
                                    def: def,
                                    segments: hir_vec![
                ...
                PatKind::TupleStruct(ref path, ref pats, ddpos) => {
                    hir::PatKind::TupleStruct(self.lower_path(p.id, path, ParamMode::Optional),
                                              pats.iter().map(|x| self.lower_pat(x)).collect(),
                                              ddpos)
                }
                PatKind::Path(ref qself, ref path) => {
                    self.lower_qpath(p.id, qself, path, ParamMode::Optional,
                                     hir::PatKind::Path, hir::PatKind::Project)
                }
                PatKind::Struct(ref pth, ref fields, etc) => {
                    let pth = self.lower_path(p.id, pth, ParamMode::Optional);
                    let fs = fields.iter()
                                   .map(|f| {
                                       Spanned {
                                           span: f.span,
        ...
        P(hir::Expr {
            ...
            node: match e.node {
                ...
                ExprKind::Path(ref qself, ref path) => {
                    self.lower_qpath(e.id, qself, path, ParamMode::Optional,
                                     hir::ExprPath, hir::ExprProject)
                }
                ExprKind::Break(opt_ident) => {
                    hir::ExprBreak(self.lower_label(e.id, opt_ident))
                }
                ExprKind::Continue(opt_ident) => {
                    hir::ExprAgain(self.lower_label(e.id, opt_ident))
                }
                ...
                ExprKind::Struct(ref path, ref fields, ref maybe_expr) => {
                    hir::ExprStruct(self.lower_path(e.id, path, ParamMode::Optional),
    ...
    fn expr_ident(&mut self, span: Span, id: Name, binding: NodeId) -> P<hir::Expr> {
        ...
        let expr_path = hir::ExprPath(None, hir::Path {
            span: span,
            global: false,
            def: def,
    ...
    fn expr_std_path(&mut self,
        ...
        let def = path.def;
        let expr = self.expr(span, hir::ExprPath(None, path), attrs);
        self.resolver.record_resolution(expr.id, def);
        expr
    }
    ...
    fn expr_std_struct(&mut self,
        ...
        let def = path.def;
        let expr = self.expr(span, hir::ExprStruct(path, fields, e), attrs);
        self.resolver.record_resolution(expr.id, def);
        expr
    }
    ...
    fn std_path(&mut self, span: Span, components: &[&str], is_value: bool) -> hir::Path {
        ...
        let segments: Vec<_> = idents.map(|name| {
            hir::PathSegment::from_name(token::intern(name))
        }).collect();
