...
pub fn lookup_in_trait(
        ...
        self_expr: Option<&ast::Expr>,      // The expression `a`, if available.
        m_name: ast::Name,                  // The name `b`.
...
impl<'a, 'b> LookupContext<'a, 'b> {
    ...
    fn confirm_candidate(&self, rcvr_ty: ty::t, candidate: &Candidate)
        ...
        debug!("confirm_candidate({})", self.cand_to_str(candidate));
        ...
        let (_, fn_sig) = replace_bound_regions_in_fn_sig( tcx, &fn_sig, |br| {
            self.fcx.infcx().next_region_var(
                infer::BoundRegionInFnCall(self.span, br))
        });
