...
fn try_overloaded_deref(fcx: @FnCtxt,
    ...
    let method = match (lvalue_pref, fcx.tcx().lang_items.deref_mut_trait()) {
        (PreferMutLvalue, Some(trait_did)) => {
            method::lookup_in_trait(fcx, span, base_expr, token::intern("deref_mut"),
    ...
    let method = match (method, fcx.tcx().lang_items.deref_trait()) {
        (None, Some(trait_did)) => {
            method::lookup_in_trait(fcx, span, base_expr, token::intern("deref"),
                                    trait_did, base_ty, [], DontAutoderefReceiver)
        }
        (method, _) => method
    };
...
pub fn impl_self_ty(vcx: &VtableContext,
    ...
    let rps = vcx.infcx.next_region_vars(infer::BoundRegionInTypeOrImpl(span),
                                         n_rps);
