...
#![feature(core, step_trait, unboxed_closures, zero_one)]
...
impl std::num::One for NaiveDate {
    fn one() -> NaiveDate {
        NaiveDate(0, 0, 1)
    }
}
...
impl std::iter::Step for NaiveDate {
    ...
    fn steps_between(_start: &Self, _end: &Self, _by: &Self) -> Option<usize> {
        unimplemented!()
    }
}
...
where It: CloneIterator, It::Item: Clone, F: Clone {
...
trait CloneIterator: Clone+Iterator {}
impl<It: Clone+Iterator> CloneIterator for It {}
...
///
fn dates_in_year(year: i32) -> impl CloneIterator<Item=NaiveDate> {
    InGroup {
...
/// `NaiveDate`s.
///
trait DateIterator: CloneIterator<Item=NaiveDate> {}
impl<It> DateIterator for It where It: CloneIterator<Item=NaiveDate> {}
...
impl<'a, It: 'a> ByMonth<'a> for It where It: DateIterator {
    type Months = impl CloneIterator<Item=(u32, impl CloneIterator<Item=NaiveDate> + 'a)> + 'a;
...
impl<'a, It: 'a> ByWeek<'a> for It where It: DateIterator {
    type Weeks = impl CloneIterator<Item=(u32, impl CloneIterator<Item=NaiveDate> + 'a)> + 'a;
...
impl<'a, It: 'a> FormatWeeks<'a> for It
where It: Iterator, It::Item: DateIterator {
    type WeekDays = impl Iterator<Item=String> + 'a;
...
impl<'a, It: 'a> FormatMonth<'a> for It where It: DateIterator {
...
impl<'a, It: 'a> FormatMonths<'a> for It
where It: Iterator, It::Item: DateIterator + 'a {
    type Output = impl Iterator<Item=impl Iterator<Item=String> + 'a> + 'a;

    fn format_months(self) -> Self::Output {
        self.map(<_>::format_month)
    }
}
